...
<CardGroup cols={2}>
  <Card title="Crear Factura" icon="file-invoice" href="/api/create-invoice">
    Emite una factura electronica validada por el SIAT. Firma digital, CUF, PDF/XML y email automatico.
  </Card>
  <Card title="Listar Facturas" icon="list" href="/api/list-invoices">
    Lista facturas con filtros por fecha, estado, NIT y numero. Paginacion incluida.
  </Card>
  <Card title="Obtener Factura" icon="magnifying-glass" href="/api/get-invoice">
    Detalle completo de una factura: datos fiscales, CUF, estado SIAT, items y montos.
  </Card>
  <Card title="Anular Factura" icon="ban" href="/api/cancel-invoice">
...
<CardGroup cols={2}>
  <Card title="Crear Nota" icon="file-circle-minus" href="/api/credit-note">
    Emite una nota de credito o debito contra una factura validada. Endpoint dedicado con calculo automatico de montos.
  </Card>
  <Card title="Anular Nota" icon="file-circle-xmark" href="/api/cancel-note">
...
<CardGroup cols={2}>
  <Card title="Estado de Factura" icon="circle-check" href="/api/invoice-status">
    Verifica el estado actual de una factura directamente con el SIAT.
  </Card>
  <Card title="Reenviar Email" icon="paper-plane" href="/api/resend-email">
    Reenvia la factura por email al cliente o a otra direccion.
  </Card>
  <Card title="Contingencia" icon="triangle-exclamation" href="/api/events">
    Manejo de eventos offline cuando el SIAT no esta disponible.
  </Card>
  <Card title="Codigos SIAT" icon="barcode" href="/api/codes">
    Verificacion de NIT, CUFD y comunicacion con el SIAT.
  </Card>
  <Card title="Catalogos" icon="book" href="/api/catalogs">
    Sincronizacion y consulta de catalogos del SIN.
  </Card>
  <Card title="Acceso publico" icon="globe" href="/api/public-access">
    Descarga publica de PDF, XML, ticket y pagina de verificacion por CUF.
  </Card>
  <Card title="AI y RAG" icon="brain" href="/api/ai">
    Chat inteligente, validacion AI y resolucion de errores SIAT.
  </Card>
  <Card title="Storage" icon="cloud-arrow-up" href="/api/storage">
...
| `2` | `NOTE_CREDIT_DEBIT` | [Nota Credito-Debito](/api/credit-note) |
...
<Info>
  Las notas de credito/debito usan un [endpoint dedicado](/api/credit-note): `POST /api/v1/invoices/{id}/credit-note`. Los demas tipos usan el endpoint unificado `POST /api/v1/invoices`.
</Info>

Cada tipo de documento sector puede requerir campos adicionales en el request body. Ver la referencia de cada sector en la seccion [Documentos Sector](/api/sectors/sale) para los campos especificos.
