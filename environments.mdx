...
| **Costo** | Incluido en tu plan | Segun plan contratado |
...
<Card title="Desarrollo" icon="code">
  Integra la API en tu aplicacion con confianza. El sandbox replica el flujo completo de produccion.
</Card>
...
### Watermark en sandbox

Los PDFs generados en sandbox (SIAT ambiente 2) incluyen una marca de agua diagonal **"SIN VALOR LEGAL"** semitransparente sobre todo el documento. Esto aplica tanto a facturas regulares como a notas de credito/debito. En produccion (SIAT ambiente 1) los PDFs se generan sin marca de agua.
...
<Warning>
  ...
  cuando el servicio vuelve. Consulta la [guia de contingencia](/api/contingency).
</Warning>
...
<Accordion title="El sandbox tiene limites de facturas?">
  No. El sandbox esta incluido en todos los planes sin limite de facturas de desarrollo.
  No hay rate limiting estricto (solo proteccion basica contra abuso).
</Accordion>
