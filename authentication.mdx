...
<Tip>
  Activa tu plan en [app.cucu.bo/signup](https://app.cucu.bo/signup) para obtener tu API Key de sandbox.
  Las keys sandbox tienen acceso completo al SIAT piloto.
</Tip>
...
<Warning>
  **Todos los demas endpoints requieren `X-API-Key`**, incluyendo los endpoints de SIAT (`/api/v1/siat/*`),
  catalogos (`/api/v1/siat/sync/*`), facturacion (`/api/v1/invoices`), e IA (`/api/v1/ai/chat`).
  Cada endpoint valida que los recursos accedidos pertenezcan al tenant autenticado (proteccion cross-tenant).
</Warning>
